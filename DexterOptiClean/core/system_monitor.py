...
class SystemMonitor:
    ...
    def get_cpu_usage(self) -> float:
        """Get current CPU usage percentage with improved accuracy"""
        try:
    ...
    def get_memory_usage(self) -> float:
        """Get current memory usage percentage with detailed information"""
        try:
            mem = psutil.virtual_memory()
            
            # Calculate actual used memory (excluding cached/buffers on Linux)
            if self.platform != 'windows':
                # On Linux, account for cached memory which can be freed
                actual_used = mem.total - mem.available
                actual_percent = (actual_used / mem.total) * 100
                return actual_percent
            else:
                # On Windows, use the standard percent
                return mem.percent
        except Exception as e:
